...
This project uses `pyproject.toml` as the single source of truth for defining dependencies. The `uv.lock` file is a lock file generated by `uv` to ensure that every installation uses the exact same versions of the dependencies.
...
If you need to add or update dependencies, modify the `[project.dependencies]` section in `pyproject.toml`. After changing the file, update the lock file by running:
```bash
uv lock
```
Then, re-sync your environment with the new lock file:
```bash
uv pip sync
```
